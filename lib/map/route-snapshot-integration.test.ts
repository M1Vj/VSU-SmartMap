...
test("map page renders source-coordinate markers without a declutter protection registry", async () => {
  ...
  assert.doesNotMatch(pageSource, /protectedMarkerIds/);
  assert.doesNotMatch(selectionSource, /protectedMarkerIds/);
  assert.match(markersSource, /items\.map\(\(item\) => \(/);
  assert.doesNotMatch(markersSource, /displayCoordinates|spreadCoLocatedItems|zoomBucket|protectedIds/);
});
