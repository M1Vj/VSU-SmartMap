...
export function ChatOpsDashboard({ data, filters }: { data: ChatOpsDashboardData; filters?: ChatOpsFilters }) {
  ...
  const percent = (value: number) => summary.userTurns ? `${Math.round(value / summary.userTurns * 100)}%` : "0%";
  ...
  return <div className="space-y-6">
    ...
    <p className="text-xs text-muted-foreground">Metrics summarize this bounded {filters?.windowHours ?? 24}-hour page: {summary.userTurns} user turn(s) and {summary.syntheticTurns} synthetic checks. Latency and quality rates exclude synthetic checks and are not all-time traffic.</p>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <Metric label="Latency p50 / p95" value={`${summary.latencyP50Ms ?? "—"} / ${summary.latencyP95Ms ?? "—"} ms`} detail={`First delivery p50 / p95: ${summary.ttftP50Ms ?? "—"} / ${summary.ttftP95Ms ?? "—"} ms`} />
      <Metric label="Cache" value={summary.cacheHits} detail={`${percent(summary.cacheHits)} of sampled user turns`} />
